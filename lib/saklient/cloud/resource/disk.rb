...
module Saklient
  module Cloud
    module Resource
      ...
      class Disk < Saklient::Cloud::Resource::Resource
        ...
        def _on_after_api_deserialize(r, root)
          if !(r).nil?
            if !r.nil? && r.key?(:SourceDisk)
              ...
              if !(s).nil?
                id = s[:ID]
                @_source = Saklient::Cloud::Resource::Disk.new(@_client, s) if !(id).nil?
              end
            end
            if !r.nil? && r.key?(:SourceArchive)
              ...
              if !(s).nil?
                id = s[:ID]
                if !(id).nil?
                  obj = Saklient::Util::create_class_instance('saklient.cloud.resource.Archive', [@_client, s])
                  @_source = obj
                end
              end
            end
        ...
        # @return [DiskConfig]
        def create_config
          return Saklient::Cloud::Resource::DiskConfig.new(@_client, _id)
        end
        ...
        def set_size_mib(v)
          ...
          raise Saklient::Errors::SaklientException.new('immutable_field', 'Immutable fields cannot be modified after the resource creation: ' + 'Saklient::Cloud::Resource::Disk#size_mib') if !@is_new
          @m_size_mib = v
          @n_size_mib = true
          return @m_size_mib
        end
        ...
        def api_deserialize_impl(r)
          ...
          if Saklient::Util::exists_path(r, 'Server')
            @m_server = (Saklient::Util::get_by_path(r, 'Server')).nil? ? nil : Saklient::Cloud::Resource::Server.new(@_client, Saklient::Util::get_by_path(r, 'Server'))
          else
            @m_server = nil
            @is_incomplete = true
          end

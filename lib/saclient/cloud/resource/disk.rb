    ...
    module Resource
      ...
      class Disk < Saclient::Cloud::Resource::Resource
          ...
          if !(r).nil?
            ...
            if !r.nil? && r.key?(:SourceArchive)
              ...
              if !(s).nil?
                id = s[:ID]
                if !(id).nil?
                  obj = Saclient::Util::create_class_instance('saclient.cloud.resource.Archive', [@_client, s])
                  @_source = obj
                end
              end
            end
        ...
        def api_serialize_impl(withClean = false)
          ...
          raise Saclient::Errors::SaclientException.new('required_field', 'Required fields must be set before the Disk creation: ' + missing.join(', ')) if missing.length > 0
          return ret
        end

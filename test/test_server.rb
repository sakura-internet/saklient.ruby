...
class TestServer < MiniTest::Unit::TestCase
  ...
  def authorize
    config = load_config
    ret = Saclient::Cloud::API::authorize(config[:SACLOUD_TOKEN], config[:SACLOUD_SECRET])
    ret = ret.inZone(config[:SACLOUD_ZONE]) if config[:SACLOUD_ZONE]
    assert_instance_of(Saclient::Cloud::API, ret)
    ret
  end
